...
- **四个旋钮**：自主度（只提案 / 客观自改 / 全自动）、影响边界（禁碰的目录与操作）、运行配置（由用户显式指定后端 / 模型 / flags，不自动推断）、收敛后行为（停 / 精修 / 转目标）。
...
3. **Phase 1**：用 Phase 0 生成的 `galatea-run.sh` 启动无人值守循环（建议跑在常驻环境 / tmux 里，断连不影响）。这个脚本会固化你显式选择的后端、模型、flags 与 PATH 修正；不要依赖引擎自动猜。
   ...
   如果后端命令不在默认 `PATH`，把用户指定的 `export PATH=...:$PATH` 写进 `galatea-run.sh`。引擎会反复以全新上下文执行单轮指令；撞用量上限自动指数退避，连续多轮无进展自动熔断停机，收敛后自动退出。每轮输出落 `<目标专属目录>/logs/` 便于复盘。
...
│  ├─ galatea-run.template.sh      # 启动脚本（固化后端 / 模型 / flags）
