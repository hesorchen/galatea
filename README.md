...
<p align="center"><strong>目标驱动的自主迭代 Skill——制定一个可量化的标准，由 Agent 无人值守地反复迭代、自评、剪枝、探索、验证，逐轮逼近，直到达成目标。</strong></p>
...
3. **Phase 1**：用引擎启动无人值守循环（建议跑在常驻环境 / tmux 里，断连不影响）。**在 galatea 项目根目录执行**。引擎会自动检测后端：只找到 `claude` 就用 Claude Code，只找到 `codex` 就用 Codex；如果两者都存在，会要求你显式指定 `GALATEA_AGENT_BACKEND`：
   ...
   ```bash
   GALATEA_AGENT_BACKEND=codex bash engine/loop.sh <目标专属目录> [最大轮数]
   ```

   显式用 Claude Code 执行每轮：

   ```bash
   GALATEA_AGENT_BACKEND=claude bash engine/loop.sh <目标专属目录> [最大轮数]
   ```

   若你长期只想用其中一个，可以把 `export GALATEA_AGENT_BACKEND=codex` 或 `export GALATEA_AGENT_BACKEND=claude` 写入 shell profile。
...
- 无人值守循环依赖跳过交互式确认：Claude 默认用 `--dangerously-skip-permissions`，Codex 默认用 `codex exec --dangerously-bypass-approvals-and-sandbox`。**务必先在 Phase 0 划清影响边界**（禁 push / 禁删 / 禁碰的目录），并在隔离环境中运行。
