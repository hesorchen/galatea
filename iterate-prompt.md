...
> 任务目录（GOAL_DIR）：`<GOAL_DIR>`（本留档示例即 galatea 项目根目录）
...
3. **影响边界（禁碰）**：只在 `<GOAL_DIR>` 内动手；禁删文件（可改写，确需删除→写 pending）；禁碰 `LICENSE`；禁 `git push`、禁碰 `<GOAL_DIR>` 外层 / 父级 git；禁系统级（不装 shellcheck 等软件、不 kill 进程）；禁外发（邮件/对外 API 写/付费）。联网**读**允许。
...
- `state.md` —— 各项达标状态、待办候选、已完成 / 已否决；其达标进度表即**本轮起点基线**（上一轮全量复评写下）。
- `log.md` 最近 5 轮。
- `pending.md` —— 是否有需绕开 / 已提案的事项，避免重复提。

### 2. 取基线（不重新打分；仅第 1 轮冷启动）
本轮起点分**直接取自步骤 1 读到的 `state.md`「达标进度表」**——上一轮全量复评（步骤 5）写下、与当前产物一致，**不再重评**（省一次裁判 spawn）。**唯第 1 轮**无前序分，派独立 Judge sub-agent 按下列取证逐项打分（步骤 5 全量复评复用同一套）：
- R1：`grep -riE '<上一任务的特有术语：模型文件名 / 指标名 / 数据规模 / 版本号 / 字段名…>' templates/` —— 命中即未达标；并通读每个 template 确认是通用占位符。
...
- 输出每项「达标 / 未达标 / 待复查 + 证据」；**仅第 1 轮**落盘 `logs/round-0001/judge-pre.md`（唯一的独立 judge-pre）。
...
- 预期（哪条 rubric 项达标）
- 证伪条件（什么结果就剪枝、改方向）
查 `state.md` 的 done / rejected 去重。**优先调动现成资源**：已装的格式化 / git / code-review 类 skill、并行 sub-agent；相互独立的修复（如 R1 重写模板 与 R7 英文同步）派并行 Executor。
...
- 用步骤 2 的 R1–R8 取证**全量重打分**（不只验本轮动过的项），落盘 `logs/round-<n>/judge-post.md`，写回 `state.md` 当下一轮基线。
- 重点扫回归：改 A 有没有弄坏 B（如改路径后 README 与 SKILL 是否仍一致）。
...
- 追加 `log.md`：**轮次标题带真实时间戳**——先 `date '+%Y-%m-%d %H:%M'` 取当前时间填进 `## R<n> — <时间戳>`（不要只写日期）；正文记本轮 Judge 结论 + 假设/行动 + 稳定性验证（有对抗则记交锋）+ commit hash。
